...
% and use 1 as the usual smoothing value
%
...
% get mean maxtrix and standard deviation matrix
mu = zeros(nDims, nClasses);
sigma = zeros(nDims, nClasses);
for i = (1:nClasses),
    Y_i = Y(:, i);
    X_i = diag(Y_i) * trainFea;
    X_i = X_i(any(X_i, 2), :);
    mu(:, i) = mean(X_i)';
    sigma(:, i) = sqrt(diag(cov(X_i)));
end
...
for i = (1:size(testFea, 1)),
    copiedX_i = testFea(ones(1, nClasses), :);
    pdf = normpdf(copiedX_i', mu, sigma);
    calcY(i, :) = prod(pdf)';
end
